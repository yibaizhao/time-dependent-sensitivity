...
obs_sensF <- function(N, start_age, mean_preonset_rate, mst, 
                      indolent_rate = 0,
                      test_age=NULL, 
                      alpha, beta_t=NULL, beta_st=NULL){
...
replicate_sensF <- function(B, N, start_age, 
  ...
  foreach (i=1:B, .combine=rbind) %dopar% {
    obs_sensF(N = N, start_age = start_age, 
...
outF <- function(B, N, start_age, 
                 ...
                 indolent_rate_seq, alpha, beta_t = NULL, beta_st = NULL, saveit){
  ...
  dset_seq <- dset_seq %>% 
    by_row(function(row){
      replicate_sensF(B, N, start_age, mean_preonset_rate, 
                      mst = row$mst, test_age = row$test_age, 
                      indolent_rate = row$indolent_rate, alpha, beta_t, beta_st)
      }, .collate = "row")
  
  dset_seq <- 
    dset_seq %>% pivot_longer(cols = contains("sens"),
                             names_to = "Type",
  ...
  gg_sens <- 
    ...
    scale_color_discrete(name="Sensitivity Type",
                       ...
                       labels=c("preclinical true", "preclinical obs", 
                                "clinical true", "clinical obs",
...
plot_sens <- function(t_seq, st_seq=NULL, alpha, beta_t=NULL, beta_st=NULL, saveit){
  if(is.null(st_seq) & is.null(beta_st)){
    dset_seq <- data.frame(t = t_seq,
                       sens = sens_timeF(t=t_seq, alpha=alpha, beta_t=beta_t))
    gg_sens <- dset_seq %>%
    ...
    dset_seq <- dset_seq %>% 
      by_row(function(row){sens_timeF(t=row$t, st=row$st, alpha=alpha, beta_t=beta_t, beta_st=beta_st)},
...
generate_tb <- function(N, mean_st_seq, test_time_seq, beta_t=NULL, beta_st=NULL, saveit){
...
##################################################
control <- function(N, B,
                    mean_preonset_rate, mean_st,
                    mst_seq, indolent_rate_seq,
                    start_age, test_age_seq,
                    alpha = -2.3858, beta_st = 3.7721,
                    saveit){
  set.seed(234)
  ...
  plot_sens(t_seq = seq(0, 30, 1), st_seq = seq(2, 10, 2), 
            alpha = alpha,
            beta_st = beta_st, 
            saveit = saveit)
  # Comparison between preclinical and clinical sensitivity under indolent cases
  outF(B = B, N = 1000, 
       start_age = start_age, mean_preonset_rate = mean_preonset_rate, 
       mst_seq = mst_seq, test_age_seq = test_age_seq, 
       indolent_rate_seq = indolent_rate_seq,
       alpha = alpha, beta_t = NULL, beta_st = beta_st, 
       saveit = saveit)
