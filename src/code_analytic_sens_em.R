...
get.dx=function(rate.matrix,
  ...
  obsout <- foreach(t=discreteout$obs.times, .combine=rbind) %dopar%  {
    ...
    sensitivity <- test_sensitivity(time=t-outctmc$times[pre.clinical.cancer.state], # test time-preclinical onset time
    ...
    dset_obs$true.sens <- ifelse((t-outctmc$times[pre.clinical.cancer.state]) >= sojourn_time | (t-outctmc$times[pre.clinical.cancer.state]) < 0, NA, sensitivity)
    dset_obs$onset.times <- outctmc$times[pre.clinical.cancer.state]
    dset_obs
  }
...
empirical.sensitivity.simulation <- function(seed=123, 
  ...
  out_sens <- out %>%
    ...
    summarise(screen=sum(result==2, na.rm=TRUE),
              ...
              mean_true_sensitivity=mean(true.sens, na.rm = TRUE))
  return(out_sens)
}
