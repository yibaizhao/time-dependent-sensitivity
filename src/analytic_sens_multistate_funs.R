...
f_es_cl <- function(t, t0, s1, s2, m1, m2, test = TRUE){
  ...
  # m1: mst from early stage to clinical stage
  # m2: mst from early stage to late stage
  f_joint <- g(s = s1, mean_sojourn_time = m1) * g(s = s2, mean_sojourn_time = m2)
  if(test){
...
f_es_ls <- function(t, t0, s1, s2, m1, m2, test = TRUE){
  # t0: time at early stage
  ...
  if(test){
    f_joint <- f_joint * h(t = (t - t0), sojourn_time = s2, lower = 0, upper = 0.8)
  }
  return(f_joint)
}
# integral over s2
test_es_ls_integral_s2 <- function(t, t0, s1, m1, m2){
  integrate(f = f_es_ls, 
            lower = t-t0, upper = s1, 
            t = t, t0 = t0, s1 = s1,
            m1 = m1, m2 = m2)$value
...
# integral over s1
test_es_ls_integral_s1 <- function(t, t0, m1, m2){
  integrate(f = Vectorize(test_es_ls_integral_s2), 
            lower = t-t0, upper = Inf,
            t = t, t0 = t0,
            m1 = m1, m2 = m2)$value
}
...
f_es_ls_integral_t0 <- function(t, t0, preonset_rate, m1, m2){
  f(u = t0, preonset_rate) * test_es_ls_integral_s1(t, t0, m1, m2)
}
...
f_ls_cl <- function(t, t0, s1, s2, s3, m1, m2, m3, test = TRUE){
  # t0: time at early stage
  # t1: time at early stage clinical onset
  # t2: time at late stage
  # m1: mst from early stage to clinical stage
...
# integral over s3
test_ls_cl_integral_s3 <- function(t, t0, s1, s2, m1, m2, m3){
  integrate(f = f_ls_cl, 
            lower = t-(t0+s2), upper = Inf, 
            t = t, t0 = t0, s1 = s1, s2 = s2,
            m1 = m1, m2 = m2, m3 = m3)$value
}
# integral over s2
test_ls_cl_integral_s2 <- function(t, t0, s1, m1, m2, m3){
  integrate(f = Vectorize(test_ls_cl_integral_s3), 
            lower = t-t0, upper = s1,
            t = t, t0 = t0, s1 = s1, 
            m1 = m1, m2 = m2, m3 = m3)$value
}
# integral over s1
test_ls_cl_integral_s1 <- function(t, t0, m1, m2, m3){
  integrate(f = Vectorize(test_ls_cl_integral_s2), 
            lower = t-t0, upper = Inf,
            t = t, t0 = t0, 
            m1 = m1, m2 = m2, m3 = m3)$value
}
...
f_ls_cl_integral_t0 <- function(t, t0, preonset_rate, m1, m2, m3){
  f(u = t0, preonset_rate) * test_ls_cl_integral_s1(t, t0, m1, m2, m3)
}
...
# integral over s2
case_es_ls_integral_s2 <- function(t, t0, s1, m1, m2){
  integrate(f = f_es_ls, 
            lower = t-t0, upper = s1, 
            t = t, t0 = t0, s1 = s1,
            m1 = m1, m2 = m2,
...
# integral over s1
case_es_ls_integral_s1 <- function(t, t0, m1, m2){
  integrate(f = Vectorize(case_es_ls_integral_s2), 
            lower = t-t0, upper = Inf,
            t = t, t0 = t0,
            m1 = m1, m2 = m2)$value
}
...
f_case_es_ls_integral_t0 <- function(t, t0, preonset_rate, m1, m2){
  f(u = t0, preonset_rate) * case_es_ls_integral_s1(t, t0, m1, m2)
}
...
# integral over s3
case_ls_cl_integral_s3 <- function(t, t0, s1, s2, m1, m2, m3){
  integrate(f = f_ls_cl, 
            lower = t-(t0+s2), upper = Inf, 
            t = t, t0 = t0, s1 = s1, s2 = s2,
            m1 = m1, m2 = m2, m3 = m3,
...
# integral over s2
case_ls_cl_integral_s2 <- function(t, t0, s1, m1, m2, m3){
  integrate(f = Vectorize(case_ls_cl_integral_s3), 
            lower = t-t0, upper = s1,
            t = t, t0 = t0, s1 = s1, 
            m1 = m1, m2 = m2, m3 = m3)$value
}
# integral over s1
case_ls_cl_integral_s1 <- function(t, t0, m1, m2, m3){
  integrate(f = Vectorize(case_ls_cl_integral_s2), 
            lower = t-t0, upper = Inf,
            t = t, t0 = t0, 
            m1 = m1, m2 = m2, m3 = m3)$value
}
...
f_case_ls_cl_integral_t0 <- function(t, t0, preonset_rate, m1, m2, m3){
  f(u = t0, preonset_rate) * case_ls_cl_integral_s1(t, t0, m1, m2, m3)
}
...
# case_ls_cl_integral(t, preonset_rate, m1, m2, m3)
