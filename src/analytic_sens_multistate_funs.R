...
#########
mst_es_cl_f <- function(t0, t1, t2, m1, m2){
  ...
  (t1 - t0) * g(s = (t1 - t0), mean_sojourn_time = m1) * g(s = (t2 - t0), mean_sojourn_time = m2)
}

mst_es_cl_integral_t1 <- function(t0, t2, m1, m2) {
  integrate(f = mst_es_cl_f, lower = t0, upper = t2, 
                                 t0 = t0, t2 = t2,
                                 m1 = m1, m2 = m2)$value
}
# Compute the nested integral
mst_es_cl_integral_t2 <- function(t0, m1, m2){
  integrate(f = Vectorize(mst_es_cl_integral_t1), 
                 lower = t0, upper = Inf,
                 t0 = t0,
                 m1 = m1, m2 = m2)$value
  }
...
#########
mst_es_ls_f <- function(t0, t1, t2, m1, m2){
  # t0: time at early stage
  # t1: time at early stage clinical onset
  # t2: time at late stage
  # m1: mst from early stage to clinical stage
  # m2: mst from early stage to late stage
  (t2 - t0) * g(s = (t1 - t0), mean_sojourn_time = m1) * g(s = (t2 - t0), mean_sojourn_time = m2)
}

mst_es_ls_integral_t2 <- function(t0, t1, m1, m2) {
...
# Compute the nested integral
mst_es_ls_integral_t1 <- function(t0, m1, m2){
  integrate(f = Vectorize(mst_es_ls_integral_t2), 
            lower = t0, upper = Inf,
            t0 = t0,
            m1 = m1, m2 = m2)$value
}
...
f_es_cl <- function(t, t0, t1, t2, m1, m2, test = TRUE){
  # t0: time at early stage
  # t1: time at early stage clinical onset
  # t2: time at late stage
  # m1: mst from early stage to clinical stage
  ...
  if(test){
    f_joint <- f_joint * h(t = (t - t0), sojourn_time = (t1 - t0), lower = 0, upper = 0.3)
  }
  return(f_joint)
}
# integral over t1
test_es_cl_integral_t1 <- function(t, t0, t2, m1, m2){
  integrate(f = f_es_cl, 
            lower = t, upper = t2, 
            t = t, t0 = t0, t2 = t2,
            m1 = m1, m2 = m2)$value
...
# integral over t2
test_es_cl_integral_t2 <- function(t, t0, m1, m2){
  integrate(f = Vectorize(test_es_cl_integral_t1), 
            lower = t, upper = Inf,
            t = t, t0 = t0,
            m1 = m1, m2 = m2)$value
}
...
f_es_cl_integral_t0 <- function(t, t0, preonset_rate, m1, m2){
  f(u = t0, preonset_rate) * test_es_cl_integral_t2(t, t0, m1, m2)
}
...
# integral over t1
case_es_cl_integral_t1 <- function(t, t0, t2, m1, m2){
  integrate(f = f_es_cl, 
            lower = t, upper = t2, 
            t = t, t0 = t0, t2 = t2,
            m1 = m1, m2 = m2,
...
# integral over t2
case_es_cl_integral_t2 <- function(t, t0, m1, m2){
  integrate(f = Vectorize(case_es_cl_integral_t1), 
            lower = t, upper = Inf,
            t = t, t0 = t0,
            m1 = m1, m2 = m2)$value
}
...
f_case_es_cl_integral_t0 <- function(t, t0, preonset_rate, m1, m2){
  f(u = t0, preonset_rate) * case_es_cl_integral_t2(t, t0, m1, m2)
}
